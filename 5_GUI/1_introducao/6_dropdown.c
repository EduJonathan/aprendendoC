...
/**
 * dropdown
 * Dropdown é um tipo de menu que exibe uma lista de opções quando clicado. Ele é útil para economizar espaço na interface,
 * permitindo que o usuário escolha uma opção sem precisar de muitos botões ou campos visíveis ao mesmo tempo.
 * 
 * Neste exemplo, criamos um dropdown para selecionar a classe de um personagem. O usuário pode clicar no dropdown
 * para abrir a lista de opções, escolher uma classe e clicar no botão "Criar Personagem" para ver um resumo da escolha.
 * O dropdown é implementado usando retângulos para a caixa principal e para cada opção, e a interação é gerenciada com eventos de mouse.
*/
...
int main(int argc, char **argv)
{
    ...
    SDL_Window *win = SDL_CreateWindow("Criação de Personagem", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, W, H, SDL_WINDOW_SHOWN);
        ...
        while (SDL_PollEvent(&e))
        {
            ...
            if (e.type == SDL_MOUSEBUTTONDOWN && e.button.button == SDL_BUTTON_LEFT)
            {
                ...
                if (SDL_PointInRect(&mouse, &button))
                {
                    ...
                    {
                        snprintf(summary, sizeof(summary), "Personagem criado!\nNome: %s\nClasse: %s", name, dd.options[dd.selected]);
                        created = 1;
                    }
                }
     ...
     * gcc -o criacao 6_dropdown.c `sdl2-config --cflags --libs` -lSDL2 -lSDL2_ttf
     * ./criacao
     */
    return 0;
}
